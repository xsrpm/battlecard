...
class Jugador {
  ...
  cogerUnaCartaDelDeck () {
    if (this.mano.length === Jugador.MAX_MANO_CARDS) { return { resultado: ResultadoCojerUnaCarta.MANO_LLENA } }
  ...
  puedeColocarCartas () {
    if (!this.puedeColocarCartaEnZB) { return 'Ya colocó cartas en este turno' }
    if (this.nCartasEnZB === Jugador.MAX_ZONA_BATALLA_CARDS) { return 'La zona de batalla está llena' }
    return 'Posible'
  }
  ...
  puedeColocarCartaDesdeId (idCartaMano) {
    ...
    if (!this.tieneCartaEnMano(idCartaMano)) { return 'No hay carta en la mano para esa posicion' }
    return 'Posible'
  }
  ...
  accionColocarCarta (idPosZB, idCartaMano, posBatalla) {
    ...
    if (rpta.resultado === 'Posible') {
      ...
      if (posBatalla === CeldaBatalla.Estado.POS_BATALLA_ATAQUE) { this.nAtaquesDisponibles++ }
  ...
  puedeAtacarCartas (jugadorAtacado) {
    if (this.nTurnos < 2) { return 'Atacantes solo se pueden realizar desde el segundo turno' }
    if (this.nCartasEnZB === 0) return 'Sin cartas en zona de batalla'
    if (jugadorAtacado.nCartasEnZB === 0) { return 'No hay cartas en zona de batalla enemiga' }
    if (this.nAtaquesDisponibles === 0) { return 'No le quedan ataques disponibles' }
    if (jugadorAtacado.barrera.length === 0) { return 'Jugador enemigo debe tener barreras' }
    return 'Posible'
  }
  ...
  puedeAtacarCartaDesdeId (jugadorAtacado, idZonaBatalla) {
    ...
    if (resp !== 'Posible') return resp
    if (this.existeCartaEnCeldaBatalla(idZonaBatalla) !== true) { return 'No hay carta en tu ubicación de zona de batalla' }
    if (
  ...
  posibilidadAtacarCarta (jugadorAtacado, idCartaAtacada, idCartaAtacante) {
    ...
    if (resp !== 'Posible') return resp
    if (jugadorAtacado.zonaBatalla[idCartaAtacada].carta === null) { return 'No hay carta en ubicación de zona de batalla enemiga' }
    return 'Posible'
  }
  ...
  puedeCambiarPosicion () {
    if (this.nCartasEnZB === 0) return 'Sin cartas en zona de batalla'
    if (this.nCambiosPosicionesDisponibles === 0) { return 'Sin cambios de posición disponibles' }
    return 'Posible'
  }
  ...
  posibilidadCambiarPosicionBatallaEnCarta (idZonaBatalla) {
    ...
    if (resp !== 'Posible') return resp
    if (this.zonaBatalla[idZonaBatalla].carta === null) { return 'No hay carta en posición de batalla indicada' }
    if (
