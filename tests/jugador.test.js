...
describe('Jugador clase', () => {
    test('tiene propiedades estaticas válidas', () => {
        ...
        expect(Jugador.MAX_DECK).toBe(Carta.MAX_VALOR_CARTA * Carta.NUMERO_ELEMENTOS_CARTAS)
        ...
        expect(Jugador.VeredictoAtaque.ENEMIGO_SIN_BARRERA).toBe('ENEMIGO SIN BARRERA')
...
describe('Jugador objeto', () => {
    ...
    test('inicia turno válido', () => {
        jugador.zonaBatalla[1].agregarCarta(carta, CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
        jugador.zonaBatalla[2].agregarCarta(carta, CeldaBatalla.Estado.POS_BATALLA_DEF_ARRIBA)
        jugador.iniciarTurno()
        expect(jugador.zonaBatalla[0].carta).toBeNull()
        expect(jugador.zonaBatalla[1].dispAtaque).toBe(CeldaBatalla.Estado.ATAQUE_DISPONIBLE)
        expect(jugador.zonaBatalla[2].dispAtaque).toBe(CeldaBatalla.Estado.ATAQUE_NO_DISPONIBLE)
    ...
    describe('posibilidad colocar carta en posicion', () => {
        test('no está habilitado para colocar carta',()=>{
            ...
            expect(jugador.posibilidadColocarCartaEnPosicion(0,0)).toBe('No está habilitado para colocar carta')
        })
        test('no, no hay carta en la mano para esa posicion',()=>{
            expect(jugador.posibilidadColocarCartaEnPosicion(0,0)).toBe('No hay carta en la mano para esa posicion')
        })
        test('no, posición en zona de batalla está ocupada', () => {
            jugador.mano.push(carta)
            jugador.zonaBatalla[0].agregarCarta(carta,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            expect(jugador.posibilidadColocarCartaEnPosicion(0,0)).toBe('Posición en zona de batalla está ocupada')
        });
        test('posible',()=>{
            jugador.mano.push(carta)
    ...
    describe('accion Colocar Carta', () => {
        test('posible, en posición de ataque', () => {
            ...
            expect(jugador.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)).toBe('Posible')
        ...
        test('posible, en otras posiciones', () => {
            ...
            jugador.nAtaquesDisponibles=0
            jugador.nCartasEnZB=0
            expect(jugador.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_DEF_ABAJO)).toBe('Posible')
            expect(jugador.puedeColocarCartaEnZB).toBe(false)
            expect(jugador.nCartasEnZB).toBe(1) 
            expect(jugador.nAtaquesDisponibles).toBe(0)
        });
        test('no posible', () => {
            expect(jugador.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)).not.toBe('Posible')
        });
    });
    ...
    describe('puede Atacar Barreras', () => {
        ...
        test('no, sin cartas en zona de batalla', () => {
            expect(jugador.puedeAtacarBarreras(jugadorEnemigo)).toBe('Sin cartas en zona de batalla')
        });
        test('no, hay cartas en zona de batalla enemiga', () => {
            jugador.nCartasEnZB = 1
            jugadorEnemigo.nCartasEnZB = 1
            expect(jugador.puedeAtacarBarreras(jugadorEnemigo)).toBe('Hay cartas en zona de batalla enemiga')
        });
        test('no, no le quedan ataques disponibles', () => {
            jugador.nCartasEnZB = 1
            jugador.nAtaquesDisponibles = 0
            expect(jugador.puedeAtacarBarreras(jugadorEnemigo)).toBe('No le quedan ataques disponibles')
        });
        test('no, en primer turno no se puede atacar barreras', () => {
            jugador.nCartasEnZB = 1
            jugador.nAtaquesDisponibles = 1
            jugador.nTurnos = 1
            expect(jugador.puedeAtacarBarreras(jugadorEnemigo)).toBe('En primer turno no se puede atacar')
        });
        test('posible', () => {
            jugador.nCartasEnZB = 1
            jugador.nAtaquesDisponibles = 1
    ...
    describe('posibilidad de Atacar Barrera carta en posicion', () => {
        ...
        test('no puede atacar barreras', () => {
            expect(jugador.posibilidadAtacarBarrera(jugadorEnemigo,0)).not.toBe('Posible')
        });
        test('no hay carta en tu ubicación de zona de batalla', () => {
            ...
            jugador.nAtaquesDisponibles=1
            expect(jugador.posibilidadAtacarBarrera(jugadorEnemigo,0)).toBe('No hay carta en tu ubicación de zona de batalla')
        });
        test('carta atacante no está en posición de ataque', () => {
            jugador.mano.push(carta)
            jugador.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_DEF_ABAJO)
            jugador.nAtaquesDisponibles=1
            expect(jugador.posibilidadAtacarBarrera(jugadorEnemigo,0)).toBe('Carta atacante no está en posición de ataque')
        });
        test('carta atacante no tiene ataques disponibles', () => {
            jugador.mano.push(carta)
            jugador.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            jugador.nAtaquesDisponibles=1
            jugador.zonaBatalla[0].dispAtaque= CeldaBatalla.Estado.ATAQUE_NO_DISPONIBLE
            expect(jugador.posibilidadAtacarBarrera(jugadorEnemigo,0)).toBe('Carta atacante no tiene ataque disponible')
        });
        test('Posible', () => {
            jugador.mano.push(carta)
            jugador.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            jugador.nAtaquesDisponibles=1
            expect(jugador.posibilidadAtacarBarrera(jugadorEnemigo,0)).toBe('Posible')
        });
    });
    ...
    describe('accion atacar barrera', () => {
        ...
        test('Posible, barrera destruida', () => {
            ...
            jugadorEnemigo.barrera.push(carta)
            expect(jugador.accionAtacarBarrera(jugadorEnemigo,0)).toBe('BARRERA DESTRUIDA')
        ...
        test('Posible, enemigo sin barreras', () => {
            ...
            jugador.nAtaquesDisponibles=1
            jugadorEnemigo.barrera.push(carta)
            expect(jugador.accionAtacarBarrera(jugadorEnemigo,0)).toBe('ENEMIGO SIN BARRERA')
    ...
    describe('puede atacar cartas', () => {
        ...
        test('no, sin cartas en zona de batalla', () => {
            expect(jugador.puedeAtacarCartas(jugadorEnemigo)).toBe('Sin cartas en zona de batalla')
        });
        test('no, no hay cartas en zona de batalla enemiga', () => {
            jugador.nCartasEnZB = 1
            jugadorEnemigo.nCartasEnZB = 0
            expect(jugador.puedeAtacarCartas(jugadorEnemigo)).toBe('No hay cartas en zona de batalla enemiga')
        });
        test('no, no le quedan ataques disponibles', () => {
            jugador.nCartasEnZB = 1
            jugador.nAtaquesDisponibles = 0
            jugadorEnemigo.nCartasEnZB = 1
            expect(jugador.puedeAtacarCartas(jugadorEnemigo)).toBe('No le quedan ataques disponibles')
        });
        test('no, en primer turno no se puede atacar barreras', () => {
            jugador.nCartasEnZB = 1
            jugador.nAtaquesDisponibles = 1
            jugador.nTurnos = 1
            jugadorEnemigo.nCartasEnZB = 1
            expect(jugador.puedeAtacarCartas(jugadorEnemigo)).toBe('En primer turno no se puede atacar')
        });
        test('no, enemigo no tiene barreras', () => {
            jugador.nCartasEnZB = 1
            jugador.nAtaquesDisponibles = 1
            jugadorEnemigo.nCartasEnZB = 1
            expect(jugador.puedeAtacarCartas(jugadorEnemigo)).toBe('Jugador enemigo debe tener barreras')
        });
        test('posible', () => {
            jugador.nCartasEnZB = 1
            jugador.nAtaquesDisponibles = 1
    ...
    describe('posibilidad atacar carta', () => {
        ...
        test('no puede atacar cartas', () => {
            expect(jugador.posibilidadAtacarCarta(jugadorEnemigo,0,0)).not.toBe('Posible')
        });
        test('no hay carta en tu ubicación de zona de batalla', () => {
            jugador.mano.push(carta)
            jugador.accionColocarCarta(1,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            ...
            jugadorEnemigo.barrera.push(carta)
            jugadorEnemigo.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            expect(jugador.posibilidadAtacarCarta(jugadorEnemigo,0,0)).toBe('No hay carta en tu ubicación de zona de batalla')
        });
        test('no hay carta en ubicación de zona de batalla enemiga', () => {
            ...
            jugadorEnemigo.mano.push(carta)
            jugadorEnemigo.barrera.push(carta)
            jugadorEnemigo.accionColocarCarta(1,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            expect(jugador.posibilidadAtacarCarta(jugadorEnemigo,0,0)).toBe('No hay carta en ubicación de zona de batalla enemiga')
        });
        test('carta atacante no está en posición de ataque', () => {
            jugador.mano.push(carta)
            jugador.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_DEF_ABAJO)
            ...
            jugadorEnemigo.barrera.push(carta)
            jugadorEnemigo.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            expect(jugador.posibilidadAtacarCarta(jugadorEnemigo,0,0)).toBe('Carta atacante no está en posición de ataque')
        });
        test('carta atacante no tiene ataques disponibles', () => {
            jugador.mano.push(carta)
            jugador.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            ...
            jugadorEnemigo.barrera.push(carta)
            jugadorEnemigo.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            expect(jugador.posibilidadAtacarCarta(jugadorEnemigo,0,0)).toBe('Carta atacante no tiene ataque disponible')
        });
        test('Posible', () => {
            jugador.mano.push(carta)
            jugador.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            ...
            jugadorEnemigo.barrera.push(carta)
            jugadorEnemigo.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            expect(jugador.posibilidadAtacarCarta(jugadorEnemigo,0,0)).toBe('Posible')
        });
    ...
    describe('calculo valor ataque', () => {
        ...
        test('atacante:ESPADA atacado:TREBOL', () => {
            ...
            let {calculoVAtacante,calculoVAtacada} = jugador.calculoValorAtaque(cartaAtacante,cartaAtacada)
    ...
    describe('obtener veredicto ataque', () => {
        ...
        test('gana atacante ', () => {
            ...
            expect(jugador.obtenerVeredictoAtaque(cartaAtacante,cartaAtacada)).toBe('GANA ATACANTE')
        });
        test('pierde atacante', () => {
            cartaAtacante = new Carta(9,Carta.Elemento.CORAZON)
            cartaAtacada = new Carta(8,Carta.Elemento.ESPADA)
            expect(jugador.obtenerVeredictoAtaque(cartaAtacante,cartaAtacada)).toBe('PIERDE ATACANTE')
        });
        test('empate', () => {
            cartaAtacante = new Carta(8,Carta.Elemento.TREBOL)
            cartaAtacada = new Carta(4,Carta.Elemento.COCO)
            expect(jugador.obtenerVeredictoAtaque(cartaAtacante,cartaAtacada)).toBe('EMPATE')
        });
    });
    ...
    describe('accion atacar carta', () => {
        ...
        test('posible, jugador atacado en posición de ataque - gana atacante', () => {
            ...
            jugadorEnemigo.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            let resultadoAtaque = jugador.accionAtacarCarta(jugadorEnemigo,0,0)
            expect(resultadoAtaque.veredicto).toBe(Jugador.VeredictoAtaque.GANA_ATACANTE)
            expect(resultadoAtaque.cartaAtacante).toEqual(cartaAtacante)
            expect(jugador.nAtaquesDisponibles).toBe(0)
            expect(jugador.nCartasEnZB).toBe(1)
            expect(resultadoAtaque.estadoCartaAtacante).toBe(Jugador.EstadoCarta.ACTIVA)
            expect(resultadoAtaque.estadoCartaAtacada).toBe(Jugador.EstadoCarta.DESTRUIDA)
        ...
        test('posible, jugador atacado en posición de ataque - pierde atacante', () => {
            ...
            jugadorEnemigo.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            let resultadoAtaque = jugador.accionAtacarCarta(jugadorEnemigo,0,0)
            expect(resultadoAtaque.veredicto).toBe(Jugador.VeredictoAtaque.PIERDE_ATACANTE)
            expect(resultadoAtaque.cartaAtacante).toEqual(cartaAtacante)
            expect(jugador.nAtaquesDisponibles).toBe(0)
            expect(jugador.nCartasEnZB).toBe(0)
            expect(resultadoAtaque.estadoCartaAtacante).toBe(Jugador.EstadoCarta.DESTRUIDA)
            expect(resultadoAtaque.estadoCartaAtacada).toBe(Jugador.EstadoCarta.ACTIVA)
        ...
        test('posible, jugador atacado en posición de defensa - gana atacante', () => {
            ...
            jugadorEnemigo.barrera.push(carta)
            jugadorEnemigo.mano.push(cartaAtacada)
            jugadorEnemigo.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_DEF_ABAJO)
            let resultadoAtaque = jugador.accionAtacarCarta(jugadorEnemigo,0,0)
            expect(resultadoAtaque.veredicto).toBe(Jugador.VeredictoAtaque.GANA_ATACANTE)
            expect(resultadoAtaque.cartaAtacante).toEqual(cartaAtacante)
            expect(jugador.nAtaquesDisponibles).toBe(0)
        ...
        test('posible, jugador atacado en posición de defensa - pierde atacante', () => {
            ...
            jugadorEnemigo.barrera.push(carta)
            jugadorEnemigo.mano.push(cartaAtacada)
            jugadorEnemigo.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_DEF_ARRIBA)
            let resultadoAtaque = jugador.accionAtacarCarta(jugadorEnemigo,0,0)
            expect(resultadoAtaque.veredicto).toBe(Jugador.VeredictoAtaque.PIERDE_ATACANTE)
            expect(resultadoAtaque.cartaAtacante).toEqual(cartaAtacante)
            expect(jugador.nAtaquesDisponibles).toBe(0)
    ...
    describe('puede cambiar posicion', () => {
        test('no, sin cartas en zona de batalla', () => {
            expect(jugador.puedeCambiarPosicion()).toBe('Sin cartas en zona de batalla')
        });
        test('no, sin cambios de posición disponibles', () => {
            jugador.nCartasEnZB=1
            expect(jugador.puedeCambiarPosicion()).toBe('Sin cambios de posición disponibles')
        });
        test('posible', () => {
            jugador.nCartasEnZB=1
    ...
    describe('posibilidad cambiar posicion batalla en carta', () => {
        test('no, no puede cambiar posicion cartas', () => {
            expect(jugador.posibilidadCambiarPosicionBatallaEnCarta(0)).not.toBe('Posible')
        });
        test('no, no hay carta en posicion de batalla indicada', () => {
            jugador.nCambiosPosicionesDisponibles=1
            jugador.mano.push(carta)
            jugador.accionColocarCarta(1,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            expect(jugador.posibilidadCambiarPosicionBatallaEnCarta(0)).toBe('No hay carta en posición de batalla indicada')
        });
        test('no, carta indicada no tiene disponible el cambio de posición', () => {
            jugador.nCambiosPosicionesDisponibles=1
            jugador.mano.push(carta)
            jugador.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            expect(jugador.posibilidadCambiarPosicionBatallaEnCarta(0)).toBe('Carta indicada no tiene disponible el cambio de posición')
        });
        test('posible', () => {
            jugador.nCambiosPosicionesDisponibles=1
            jugador.mano.push(carta)
            jugador.accionColocarCarta(0,0,CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            jugador.zonaBatalla[0].dispCambio = CeldaBatalla.Estado.CAMBIO_POS_DISPONIBLE
            expect(jugador.posibilidadCambiarPosicionBatallaEnCarta(0)).toBe('Posible')
        });
    });
    ...
    describe('cambiar posicion batalla', () => {
        ...
        test('posible, a posición de ataque', () => {
            ...
            expect(jugador.zonaBatalla[0].posBatalla).toBe(CeldaBatalla.Estado.POS_BATALLA_ATAQUE)
            expect(jugador.zonaBatalla[0].dispCambio).toBe(CeldaBatalla.Estado.CAMBIO_POS_NO_DISPONIBLE)
            expect(jugador.zonaBatalla[0].dispAtaque).toBe(CeldaBatalla.Estado.ATAQUE_DISPONIBLE)
            expect(jugador.nCambiosPosicionesDisponibles).toBe(0)
        });
        test('posible, a posición de defensa', () => {
            ...
            jugador.zonaBatalla[0].dispCambio = CeldaBatalla.Estado.CAMBIO_POS_DISPONIBLE
            expect(jugador.cambiarPosicionBatalla(0)).toBe('Posible')
            expect(jugador.zonaBatalla[0].posBatalla).toBe(CeldaBatalla.Estado.POS_BATALLA_DEF_ARRIBA)
            expect(jugador.zonaBatalla[0].dispCambio).toBe(CeldaBatalla.Estado.CAMBIO_POS_NO_DISPONIBLE)
            expect(jugador.zonaBatalla[0].dispAtaque).toBe(CeldaBatalla.Estado.ATAQUE_NO_DISPONIBLE)
            expect(jugador.nCambiosPosicionesDisponibles).toBe(0)
        });
    });
